...
class _AiInstallPageState extends State<AiInstallPage>with AutomaticKeepAliveClientMixin {
  // late final List<DownloadController> _downloadControllers;
  List dataList = [];
  Map<String,dynamic> appJson={"data":[{"list":[],"type":"图片相关"}]};
  getData() {
          ...
          (index) => SliverStickyHeader.builder(
                ...
                sliver: SliverList(
                  delegate: SliverChildBuilderDelegate(
                    (context, i) => _appWidget(i, dataList[index]['list'],dataList[index]['type']),
  ...
  Widget _appWidget(index, _dataList,groupName) {
    ...
    var downloadController = SimulatedDownloadController(
        onOpenDownload: () async {
          print('${appDocumentsDir.path}/AiTools/${_dataList[index]['name']}/${path.basename(_dataList[index]['url'])}');
          ...
          if (outputFile != null) {
              ...
              extractFileToDisk('${appDocumentsDir.path}/AiTools/${_dataList[index]['name']}/${path.basename(_dataList[index]['url'])}', '$outputFile/');
              String appData= prefs.getString("app")??json.encode(appJson);
               ...
               appJson['data'].forEach((element) {
                 ...
                 if(element['type']!=groupName)
                   {
                     appJson['data'].add({"list":[{"name":_dataList[index]['name'],"desc":_dataList[index]['desc'],"url":"$outputFile/${path.basename(_dataList[index]['url']).split('.')[0]}","icon":_dataList[index]['icon'],"version":_dataList[index]['version']}],"type":groupName});
                   }else{
                    if(element['list'].isNotEmpty){
                      element['list'].forEach((elem) {
                        if(elem['name']!=_dataList[index]['name']){
                          element['list'].add({"name":_dataList[index]['name'],"desc":_dataList[index]['desc'],"url":"$outputFile/${path.basename(_dataList[index]['url']).split('.')[0]}","icon":_dataList[index]['icon'],"version":_dataList[index]['version']});
                        }
                      });
                    }else{
                      element['list']=[{"name":_dataList[index]['name'],"desc":_dataList[index]['desc'],"url":"$outputFile/${path.basename(_dataList[index]['url']).split('.')[0]}","icon":_dataList[index]['icon'],"version":_dataList[index]['version']}];
                    }
        ...
        downloadName: '${_dataList[index]['name']}')..getDownloadStatus();
    return ListTile(
